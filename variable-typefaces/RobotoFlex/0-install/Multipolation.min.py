...
#-*- coding: utf-8 -*-
__doc__="""
(GUI) Advanced interpolation with infinite axes.
"""
  ...
  for MasterWithChildsName2 in MastersWithChilds:
   if MasterWithChildsName2 in children:
    ...
    if(firstIndex>secondIndex):
     k+=1
     OrderAsArray.insert(firstIndex,OrderAsArray.pop(secondIndex))
     NewOrderAsArray=OrderAsArray
 ...
 for instancename in data["InstancesSetup"]:
  if not instancename.startswith("_"):
   if not("#All" in data["InstancesSetup"][instancename]):
    ...
    data["InstancesSetup"][instancename][charGroupName]=tempdata["InstancesSetup"][instancename]
   for charGroupName in data["InstancesSetup"][instancename]:
    if not(charGroupName=="#All"):
     ...
     data["InstancesSetup"][instancename][charGroupName]=tempdata["InstancesSetup"][instancename]["#All"]
   ...
   for charGroupName in data["InstancesSetup"][instancename]:
    if not charGroupName.startswith("_"):
     for mastername in data["InstancesSetup"][instancename][charGroupName]:
      if not mastername.startswith("_"):
       interpolvals=data["InstancesSetup"][instancename][charGroupName][mastername]
       if not(type(interpolvals)==list):
        valuestr=str(interpolvals)
        tempMin=str(data["MasterSetupMapping"][mastername][0][0])
        tempMax=str(data["MasterSetupMapping"][mastername][0][1])
        tempHalf=str(map(0.5,0,1,eval(tempMin),eval(tempMax)))
        ...
        data["InstancesSetup"][instancename][charGroupName][mastername]=[interpolval,interpolval]
       else:
        new_interpolvals=[]
        for k in range(len(interpolvals)):
         valuestr=str(interpolvals[k])
         tempMin=str(data["MasterSetupMapping"][mastername][k][0])
         tempMax=str(data["MasterSetupMapping"][mastername][k][1])
         tempHalf=str(map(0.5,0,1,eval(tempMin),eval(tempMax)))
         valuetemp=valuestr.replace("parent",tempMin)
         valuestr=str(valuetemp)
         valuetemp=valuestr.replace("none",tempMin)
        ...
        data["InstancesSetup"][instancename][charGroupName][mastername]=[new_interpolvals[0],new_interpolvals[1]]
"""Main Function/Class"""
class Multipolation(object):
 ...
 def LoadSpecification(self,jsonpath):
  ...
  print "----------------------------------------------------------------------"
  print "\n   Read Spec File = ", jsonpath.split("/")[-1]
 ...
 def Interpolation(self,data):
  ...
  Standard_Filters_Before=data["GeneralSetup"]["Filters_general_before"]
  ...
  Instances_reset_all=data["GeneralSetup"]["Instances_reset_all"]
  Add_Custom_Parameter=len(Standard_Filters_Before)or len(Standard_Filters_After)
  ...
  for k in range(0,Master_Count-1,1):
   Instance_reset_Values=Instance_reset_Values+ [0.0]
  if(Add_Masters_as_Instances):
   interpol_array.append(Instance_reset_Values+["0 - Root-Master"]+[Standard_Filters_Before])
  ...
  def process(interpol_array):
   ...
   MasterNamesCalcOrderArray=getInstanceMasterValuesCalcOrder(data)
   ...
   if(Instances_reset_all):
    ...
    if(Instances_Count!=Interpol_Count):
     raise Exception('Check Instances in the info panel (Names and Count) or activate "Instances_reset_all" in the Multipolation Spec File')
   multiplemode=1
   ...
   for instancename in data["InstancesSetup"]:
    if not instancename.startswith("_"):
     for charGroupName in data["InstancesSetup"][instancename]:
      if not charGroupName.startswith("_"):
       instanceindex=getGSObjectIndex(Instances,instancename)
       ...
       if not instancename.startswith("_"):
        ...
        for mastername in data["InstancesSetup"][instancename][charGroupName]:
         if not mastername.startswith("_"):
          ...
          test=data["InstancesSetup"][instancename][charGroupName][mastername]
          if(type(test)==dict):
           data["InstancesSetup"][instancename][charGroupName][mastername]=data["InstancesSetup"][instancename][charGroupName][mastername]["read"]
          for k in range(len(data["InstancesSetup"][instancename][charGroupName][mastername])):
           value_original_readable[k]=data["InstancesSetup"][instancename][charGroupName][mastername][k]
           masterMin=data["MasterSetupMapping"][mastername][k][0]
           masterMax=data["MasterSetupMapping"][mastername][k][1]
           interpolmapvalue_new[k]=map(value_original_readable[k],masterMin,masterMax,0,1)
           interpol_sum[k]+=interpolmapvalue_new[k]
          """assign raw value for interpolation"""
          readablevalue=value_original_readable
          data["InstancesSetup"][instancename][charGroupName].update({mastername:{"read":value_original_readable,"map":interpolmapvalue_new,"calc":[0,0],"childsum":[0,0]}})
       Instance.setManualInterpolation_(True)
       ...
       for mastername in MasterNamesCalcOrderArray:
        if not mastername.startswith("_"):
         ...
         if(len(data["MasterSetupMapping"][mastername])>2):
          children=data["MasterSetupMapping"][mastername][-1]
          if(children=="All"):
           children=getMasterNames(mastername)
           data["MasterSetupMapping"][mastername][-1]=children
          masterNameArray=getMasterNames("")
          masterselection=children
          interpol_child_sum=sumSelDictValues(data["InstancesSetup"][instancename][charGroupName],masterselection,"calc")
          data["InstancesSetup"][instancename][charGroupName][mastername]["childsum"]=redefinelist(interpol_child_sum)
          for k in range(len(interpolcalcvalue)):
           interpolcalcvalue_new[k]=interpolmapvalue[k]-interpol_child_sum[k]
         else:
          for k in range(len(interpolcalcvalue)):
           interpolcalcvalue_new[k]=interpolmapvalue[k]
          children=False
         data["InstancesSetup"][instancename][charGroupName][mastername]["calc"]=""
         data["InstancesSetup"][instancename][charGroupName][mastername]["calc"]=redefinelist(interpolcalcvalue_new)
         shit=data["InstancesSetup"][instancename][charGroupName][mastername]["calc"]
         interpolcalcvalue=interpolcalcvalue_new
         interpol_child_sum=data["InstancesSetup"][instancename][charGroupName][mastername]["childsum"]
         readablevalue=data["InstancesSetup"][instancename][charGroupName][mastername]["read"]
         readablevaluespan=[data["MasterSetupMapping"][mastername][0],data["MasterSetupMapping"][mastername][1]]
         for k in range(len(finaloverallcalcsum)):
          finaloverallcalcsum[k]+=interpolcalcvalue[k]
         masterindex=getGSObjectIndex(Masters,mastername)
         if(charGroupName=="#All"):
          interpolcalcvalue_x=interpolcalcvalue[0]
          interpolcalcvalue_y=interpolcalcvalue[1]
          xy_interpolation=NSValue.valueWithPoint_((interpolcalcvalue_x,interpolcalcvalue_y))
          Instance.instanceInterpolations[Masters[masterindex].id]=xy_interpolation
       if not(charGroupName=="#All"):
        for k in range(len(Font.masters)):
         mastername=byteify(Font.masters[k].name)
         interpolcalcvaluesStr=str(data["InstancesSetup"][instancename][charGroupName][mastername]["calc"]).replace("[","{").replace("]","}")
